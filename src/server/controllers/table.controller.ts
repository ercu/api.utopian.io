...
function getModeratorRow (moderator, host) {
  ...
  return '<td><center><img src="'+host+'/api/users/' + moderator.account + '/avatar?round=true&size=50" style="border-radius:50%;"><br /><a target="_blank" href="//' + process.env.UTOPIAN_DOMAIN + '/@'  + moderator.account + '">@' + moderator.account + '</a></center></td>'
}

function getSponsorRow (sponsor, host) {
  let row = '<td><center><img src="'+host+'/api/users/' + sponsor.account + '/avatar?round=true&size=50" style="border-radius:50%;"><br /><a target="_blank" href="//' + process.env.UTOPIAN_DOMAIN + '/@' + sponsor.account + '">@' + sponsor.account + '</a>'
